...
public class GoodsServiceImpl implements GoodsService {
	...
	public void delete(Long[] ids) {
		for (Long id : ids) {
			...
			goodsMapper.updateByPrimaryKeySelective(selectByPrimaryKey);
		}
	}
	...
	public PageResult findPage(TbGoods goods, int pageNum, int pageSize) {
		...
		criteria.andIsDeleteIsNull();// 指定为空的条件为未逻辑删除
		if (goods != null) {
		...
		page.getResult().forEach(ele->System.out.println(ele));
		return new PageResult(page.getTotal(), page.getResult());
	}
	...
	@Override
	public void updatecommodity(Long[] ids, String status) {
		for (Long id : ids) {

			TbGoods goods = goodsMapper.selectByPrimaryKey(id);

			if (status.equals("") || status.length() == 0) {
				goods.setIsMarketable(null);
			} else {
