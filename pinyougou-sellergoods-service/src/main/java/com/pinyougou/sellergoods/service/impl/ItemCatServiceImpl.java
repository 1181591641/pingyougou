...
public class ItemCatServiceImpl implements ItemCatService {
	...
	public List<TbItemCat> findByParenId(Long id) {
		...
		/**
		 * 商品分类页面比较特殊 执行增删改之后，都会调用该方法。进行商品分类查询
		 */

		List<TbItemCat> findAll = findAll();
		for (TbItemCat itemCat : findAll) {
			// 将全部商品放入缓存中，以商品分类名称作为kay，以模板名称作为value
	...
	public TbItemCat Names(TbItemCat itemCat) {
		...
		createCriteria.andNameEqualTo(itemCat.getName());
		return itemCatMapper.selectByExample(example).get(0);
	}
